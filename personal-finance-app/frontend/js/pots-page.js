document.addEventListener('DOMContentLoaded', function () {
    ...
    apiGet('/pots').then(function (data) {
      ...
      data.forEach(function (p) {
        ...
        html += '<section class="card">' +
          ...
          '<div class="menu-wrap"><button class="kebab" data-menu>···</button><div class="menu"><button data-open="modal-edit-pot" data-pot-id="' + p._id + '" data-pot-name="' + escapeAttr(p.name) + '" data-pot-target="' + p.target + '" data-pot-theme="' + escapeAttr(p.theme) + '">Edit Pot</button><button class="danger" data-open="modal-delete-pot" data-name="' + escapeAttr(p.name) + '" data-pot-id="' + p._id + '">Delete Pot</button></div></div></div>' +
          ...
          '<div class="pot-actions"><button class="btn btn--soft" data-open="modal-add-money" data-name="' + escapeAttr(p.name) + '" data-saved="' + p.saved + '" data-target="' + p.target + '" data-color="' + color + '" data-pot-id="' + p._id + '">+ Add Money</button><button class="btn btn--soft" data-open="modal-withdraw" data-name="' + escapeAttr(p.name) + '" data-saved="' + p.saved + '" data-target="' + p.target + '" data-pot-id="' + p._id + '">Withdraw</button></div>' +
          '</section>';
      });
  ...
  document.addEventListener('click', function (e) {
    ...
    if (openEdit && openEdit.dataset.potId) {
      ...
      modal.querySelector('[name="pot-name"]').value = openEdit.dataset.potName || '';
      modal.querySelector('[name="pot-target"]').value = openEdit.dataset.potTarget || '';
      modal.querySelector('[name="pot-theme"]').value = themeLabel(openEdit.dataset.potTheme || 'green');
    }
    ...
    if (editBtn) {
      ...
      var target = parseFloat(editModal.querySelector('[name="pot-target"]').value);
      var theme = themeValue(editModal.querySelector('[name="pot-theme"]').value);
      if (id && name && target && theme) {
        apiPut('/pots/' + id, { name: name, target: target, theme: theme }).then(function () { load(); });
      }
    }
    ...
    if (delBtn) {
      ...
      if (deleteId) apiDelete('/pots/' + deleteId).then(function () { load(); });
    }
    ...
    if (addBtn) {
      ...
      var amount = parseFloat(addModal.querySelector('[data-money-input]').value);
      if (addId && amount > 0) {
        apiPost('/pots/' + addId + '/add', { amount: amount }).then(function () { load(); });
      }
    }
    ...
    if (wdBtn) {
      ...
      var wdAmount = parseFloat(wdModal.querySelector('[data-money-input]').value);
      if (wdId && wdAmount > 0) {
        apiPost('/pots/' + wdId + '/withdraw', { amount: wdAmount }).then(function () { load(); });
      }
    }
